  ...
  useEffect(() => {
    const cargarTrabajosInstitucion = async () => {
      ...
      try {
        ...
        let url = paisSeleccionado === 'TODOS'
          ? `${BACKEND_URL}/api/institucion/${encodeURIComponent(instId)}/trabajos`
          : `${BACKEND_URL}/api/institucion/${encodeURIComponent(paisSeleccionado)}/${encodeURIComponent(instId)}/trabajos`;
        if (consulta) {
    ...
    setPaginaActual(1);
  }, [institucionSeleccionada, paisSeleccionado, consulta, setWorks]);
